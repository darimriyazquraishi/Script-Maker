...
def estimate_topic_scope(content_plan: list, current_words: int, current_minutes: int):
    """
    Check if the discovered topic has significantly more items than can comfortably
    fit inside the user's requested duration at ~200 words/minute.
    """
    item_count = len(content_plan) if content_plan else 0
    if item_count < 7:
        return False, item_count, current_words, current_minutes

    # At ~200 wpm, covering each major item requires ~80-100 words plus 300 words for Hook/Intro/Conclusion
    recommended_words = max(current_words, item_count * 85 + 300)
    recommended_minutes = max(current_minutes, int(round(recommended_words / 200)))

    # If current target provides less than 50 words per item, it's overcrowded
    words_per_item = current_words / item_count
    needs_alert = words_per_item < 60 and item_count >= 8

    return needs_alert, item_count, recommended_words, recommended_minutes
...
def generate(
    ...
    if do_brief or existing_brief is None:
        ...
        _emit(progress, "[4/5] Generating comprehensive research brief with local AI...")
        ...
        brief = ResearchBrief(**existing_brief)
    ...
    # Token headroom calculation: ~200 words/min, generous buffer
    max_script_tokens = max(3500, min(12288, int(target_words * 2.2)))
    ...
    _emit(progress, "All requested research and script generation tasks completed successfully.")
